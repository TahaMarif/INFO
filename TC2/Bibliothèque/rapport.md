...
├──────────────────────────────────────────────┤
│ - _nom : str                                 │
│ - _conservateur : Conservateur [0..1]        │
│ - _lecteurs : list[Lecteur]                  │
│ - _livres : list[Livre]                      │
│ - _bibliothecaires : list[Bibliothecaire]    │
│ - _emprunts : list[Emprunt]                  │
├──────────────────────────────────────────────┤
│ + ajout_lecteur(nom, prenom, adresse, num)   │
│ + chercher_lecteur_numero(num) : Lecteur     │
│ + chercher_lecteur_nom(nom, prenom) : Lecteur│
│ + retrait_lecteur(num) : bool                │
│ + affiche_lecteurs()                         │
│ + ajout_livre(titre, auteur, num, nb_ex)     │
│ + chercher_livre_numero(num) : Livre         │
│ + chercher_livre_titre(titre) : Livre        │
│ + retrait_livre(num) : bool                  │
│ + affiche_livres()                           │
│ + ajout_bibliothecaire(nom, prenom, num)     │
│ + retrait_bibliothecaire(num) : bool         │
│ + chercher_bibliothecaire(num) : Bib.        │
│ + affiche_bibliothecaires()                  │
│ + emprunt_livre(num_lecteur, num_livre)      │
│ + retour_livre(num_lecteur, num_livre)       │
│ + affiche_emprunts()                         │
│ - _trouver_lecteur_par_numero(num) : Lecteur │
│ - _trouver_livre_par_numero(num) : Livre     │
│ - _trouver_bibliothecaire(num) : Bib.        │
└──────────────────────────────────────────────┘
        |                |           |         |
     0..*             0..*        0..*       0..1
  (gère)          (inscrit)   (emploie)  (dirigée par)
        |                |           |         |
┌───────────────┐ ┌─────────────┐ ┌─────────────────────┐
│    Emprunt    │ │   Lecteur   │ │   Bibliothecaire     │
├───────────────┤ ├─────────────┤ ├─────────────────────┤
│-_livre: Livre │ │-_nom: str   │ │-_nom: str            │
│-_lecteur:Lect.│ │-_prenom: str│ │-_prenom: str         │
│-_date_emprunt │ │-_adresse:str│ │-_numero: int         │
│-_date_retour  │ │-_numero: int│ ├─────────────────────┤
├───────────────┤ │-_emprunts:[]│ │+nom(): str           │
│+livre()       │ ├─────────────┤ │+prenom(): str        │
│+lecteur()     │ │+nom()       │ │+numero(): int        │
│+date_emprunt()│ │+prenom()    │ └─────────────────────┘
│+date_retour() │ │+adresse()   │          ▲
│+est_en_cours()│ │+numero()    │          │ hérite
│+enreg_retour()│ │+emprunts()  │ ┌─────────────────────┐
└──┬────────────┘ │+aj_emprunt()│ │   Conservateur       │
   │              │+ret_emprunt()│ ├─────────────────────┤
   │ concerne 1   │+a_emprunt() │ │  (pas d'attribut     │
   ▼              │+a_deja_emp()│ │   supplémentaire)    │
┌─────────────┐   └─────────────┘ └─────────────────────┘
│    Livre    │
├─────────────┤
│-_titre: str │
...
|---|---|---|
| `Bibliotheque` → `Conservateur` | Association | 1 vers 0..1 |
| `Bibliotheque` → `Lecteur` | Agrégation | 1 vers 0..* |
| `Bibliotheque` → `Livre` | Agrégation | 1 vers 0..* |
| `Bibliotheque` → `Bibliothecaire` | Agrégation | 1 vers 0..* |
| `Bibliotheque` → `Emprunt` | Composition | 1 vers 0..* |
| `Emprunt` → `Livre` | Association | 0..* vers 1 |
| `Emprunt` → `Lecteur` | Association | 0..* vers 1 |
| `Conservateur` → `Bibliothecaire` | Héritage | — |
